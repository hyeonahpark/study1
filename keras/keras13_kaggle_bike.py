...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.9, random_state=456)

#2. modeling
model=Sequential()
model.add(Dense(8, input_dim=8, activation='relu')) #activation function 활성화 함수, 한정함수 : 다음레이어에 오는 값의 범위를 한정한다. y=relu(wx+b) , relu 함수는 0보다 낮은 값이 나오면 0으로 나옴.
...
sampleSubmission.to_csv(path + "submission_0717_1.csv")
print("loss : ", loss)
print("R2의 점수 : ", r2)