...
#2. modeling
model = Sequential()
model.add(Conv2D(10, (2,2), input_shape=(10, 10, 1),  strides=1, padding='same')) #padding default 값은 valid
                        #shape = (batch_size, rows, columns, channels) #batch_size : 훈련시킬 데이터의 갯수
                        #shape = (batch_size, heights, widths, channels) #다음에 넘어갈 때는 height, widhts, filters 로 받아들임

model.add(Conv2D(filters=9, kernel_size=(3,3), strides=1, padding='same')) # 25, 25, 20
model.add(Conv2D(8, (4,4))) # 22, 22, 15
