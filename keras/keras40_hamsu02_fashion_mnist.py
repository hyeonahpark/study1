...
from tensorflow.keras.layers import Flatten, Dropout, MaxPool2D, Input
import time
...
dense1=Conv2D(64, (3,3), activation='relu', input_shape=(28, 28, 1), padding='same')(input1) #26, 26, 64
...
dense2=Conv2D(filters=64, activation='relu', kernel_size=(3,3), padding='same')(batch1) # 24, 24, 64
maxp2=MaxPool2D()(dense2)
batch2=BatchNormalization()(maxp2)
drop1=Dropout(0.3)(batch2)
dense3=Conv2D(32, (3,3), activation='relu', padding='same')(drop1) # 23, 23, 32
maxp3=MaxPool2D()(dense3)
batch3=BatchNormalization()(maxp3)
drop2=Dropout(0.3)(batch3)
flat1=Flatten()(drop2) # 모양만 바꾼거기 때문에 연산량 0  #23*23*32
dense4=Dense(units=32, activation='relu')(flat1) #None, 22, 22, 8 #Dense가 2차원이지만 2차원 이상 다 가능함
dense5=Dense(units=16, input_shape=(32, ), activation='relu')(dense4)
                        #shpae = (batch_size, input_dim)
drop3=Dropout(0.3)(dense5)
dense6=Dense(10, activation='softmax')(drop3)
...
hist=model.fit(x_train, y_train, epochs=1000, batch_size=128, validation_split=0.3, callbacks=[es])
end_time=time.time()
