...
#     verbose = 1,
#     save_best_only=True,
#     filepath='./_save/keras29_mcp/keras29_mcp1.hdf5'  #모델, 가중치 둘다 저장됨 ! 
# )


# start_time=time.time() #time.time() 현재 시간 반환
# hist = model.fit(x_train, y_train, epochs=10, batch_size=4, verbose = 1, validation_split=0.2, callbacks=[es, mcp]) #hist는 history의 약자,
# end_time=time.time() #끝나는 시간 반환

model=load_model('./_save/keras29_mcp/keras29_mcp1.hdf5') #mcp로 저장한 파일 불러오면 modeling, compile 둘다 안해도 같은 값으로 불러올 수 잇음.
