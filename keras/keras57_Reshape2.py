...
from tensorflow.keras.layers import Flatten, Dropout, LSTM
import time
...
model.add(LSTM(10))  # (N, 26, 960)
# model.add(Reshape((26, 26, 64)))  # LSTM에 입력하기 위해 Reshape
model.add(Dense(10))
model.summary()
...
hist=model.fit(x_train, y_train, epochs=100, batch_size=128, validation_split=0.3, callbacks=[es, mcp])
end_time=time.time()
