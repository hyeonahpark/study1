...
x_train, x_test, y_train, y_test = train_test_split(xy_train[0][0], xy_train[0][1], train_size=0.9, random_state=42)
...
start_time=time.time()
hist=model.fit_generate(x_train, y_train, epochs=1000, batch_size=50, validation_split=0.3, callbacks=[es, mcp])
end_time=time.time()

# model.save('./_save/keras39/k39_07/keras39_07_mcp.hdf5')

#4. 평가, 예측
loss = model.evaluate(x_test, y_test, batch_size=50, verbose=1)
print('loss :', loss[0])
print('acc :', round(loss[1],5))

y_pre = np.round(model.predict(x_test, batch_size=50))
...
### csv 파일 만들기 ###
y_submit = model.predict(xy_test, batch_size=20)
# print(y_submit)
# if y_submit >=0.5 :
#     y_submit = 1
# else :
...
sample_submission.to_csv(path_submission + "sampleSubmission_0802_24.csv")
