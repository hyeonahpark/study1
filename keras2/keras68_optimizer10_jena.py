...
for i in range(0, len(lr), 1) :
    # 2. 모델 구성
    model = Sequential()
    model.add(Dense(128, input_shape=(144*13, ), activation='relu')) # timesteps , features
    ...
    model.compile(loss='mse', optimizer=Adam(learning_rate=lr[i]), metrics=['accuracy', 'acc', 'mse'])
    ...
    path_save = 'C:\\ai5\\_save\\kears68\\k68_10\\'
    filename ='{epoch:04d}-{val_loss:.4f}.hdf5'   #1000-0.7777.hdf5
    filepath = "".join([path_save, 'k68_10_', str(i+1), '_', date, '_' , filename])
    ...
    model.fit(x_train, y_train, epochs=1000, batch_size=512, verbose =0, validation_split=0.2, callbacks=[es, mcp])
    end_time = time.time()

    #4. 평가, 예측
    loss = model.evaluate(x_test, y_test, verbose=0, batch_size=512)
    x_pred=np.array([x_pred]).reshape(144*13, 1)
    y_pred = model.predict(x_pred, batch_size=512)
    y_pred = np.array([y_pred]).reshape(144,)
