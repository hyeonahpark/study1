...
class LSTM(nn.Module):
    ...
    def forward(self, x, h0, c0):        
        h0 = torch.zeros(5, x.size(0), 64).to(DEVICE) #num_layers, batch_size, hidden_size
        c0 = torch.zeros(5, x.size(0), 64).to(DEVICE) #num_layers, batch_size, hidden_size
...
torch.save(model.state_dict(), save_path+'t23_1.pth')
"""
save_path = 'C:\\ai5\\_save\\torch\\'
...
with torch.no_grad():
    model.load_state_dict(torch.load(save_path + 't23_1.pth', map_location=DEVICE))
...
print('R2:', r2) # R2: 0.9777275323867798
print('total_loss:', total_loss.item()) #total_loss: 240.3130645751953