...
y_train = torch.LongTensor(y_train - 1).to(DEVICE)  # 1을 빼서 0부터 시작하게 변환
y_test = torch.LongTensor(y_test - 1).to(DEVICE)    # 1을 빼서 0부터 시작하게 변환
...
model = Model(54, 7).to(DEVICE)  # 출력 차원(output_dim) = 7 (다중 클래스)
